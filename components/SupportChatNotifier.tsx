...
export default function SupportChatNotifier() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      const messages: SupportNotice[] = data.messages ?? [];
      ...
      const fresh = messages.filter(
        (m) => !seenIds.current!.has(m.id) && pathname !== m.url.split("#")[0]
      );
      ...
      {toasts.map((t) => (
        ...
        >
          <div className="w-8 h-8 rounded-xl bg-violet-600/20 flex items-center justify-center shrink-0">
            <Headset size={15} className="text-violet-400" />
          </div>

          <div className="flex-1 min-w-0">
            <div className="text-xs font-bold truncate">{t.senderName}</div>
            <div className="text-xs text-gray-400 mt-0.5 truncate">{t.preview}</div>
          </div>
